...
app.config.update(
    ...
    MAIL_PORT = 465,  # Use 465 if using SSL
    MAIL_USE_TLS = False, # Use False if using SSL
    MAIL_USE_SSL = True, # Set to True if using SSL on port 465
    ...
    if request.method == 'POST':
        ...
        if len(username) > 0 and len(password) > 0 and len(email) > 0 and len(mobile) > 0 and len(fullname) > 0 and len(
            ...
            if account:
                ...
                cursor.close()
                user_email = email
                subject = "Welcome to RentTrack – Registration Successful!"
                body = f"""\
Hi {username},

Thank you for registering with RentTrack. We have received your message and will be in contact soon.

Best regards,  
RentTrack Team
                """
                mess = Message(subject=subject, sender=app.config["MAIL_USERNAME"], recipients=[user_email])
                mess.body = body
                mail.send(mess)
        ...
        if res:
            ...
            return render_template('search.html', detail="", deatail1="", detail2="", username=session['username'],
                ...
                if len(City) > 0 and len(Age) > 0 and len(Occupation) > 0 and len(Address) > 0 and len(Landmark) > 0 and len(Pincode) > 0 and len(State) > 0:
                    ...
                    mysql.connection.commit()
                    cursor.close()
                    user_email = session['email1']
                    mess = Message('Message from Admin RentTrack', sender=app.config["MAIL_USERNAME"], recipients=[user_email])
                    mess.body = f"""\
Hi {session['username']},

Thank you for your interest in purchasing a property through RentTrack.
...
Our team has received your request and will review it shortly. We will contact you soon to discuss the next steps.

If you have any questions, feel free to reach out to our support team at {app.config["MAIL_USERNAME"]}.
...
RentTrack Team  
        ...
        data = data + data1
        cur.close()
        # Construct email body for property purchase application
...
def book_apt(id):
    if 'loggedin' in session:
        if session['username'] != 'admin':
            msg=''
            mess = Message('Message from Admin RentTrack :- Meeting Booked', sender=app.config["MAIL_USERNAME"], recipients=[session["email1"]])
            cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
...
Your meeting has been successfully booked. Here are the details of your appointment:
...
Our team will be ready to assist you during the meeting. If you have any questions or need to reschedule, please contact us at {app.config["MAIL_USERNAME"]}.
...
@app.route("/delete_saved/<string:id>/<string:type>", methods=['GET', 'POST'])
def delete_saved(id, type):
    if 'loggedin' in session:
        if session['username'] != 'admin': 
            if type == 'appart':
                cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
                cursor.execute("DELETE FROM saveapartment WHERE A_ID = %s and id = %s",([id], [session['id']]))
                mysql.connection.commit()
                cursor.close()
                return redirect(url_for('dashboard'))
            elif type == 'room':
                cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
                cursor.execute("DELETE FROM saveroom WHERE R_ID = %s and id = %s",([id], [session['id']]))
                mysql.connection.commit()
                cursor.close()
                return redirect(url_for('dashboard'))
            elif type == 'project':
                cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
                cursor.execute("DELETE FROM saveproject WHERE P_ID = %s and id = %s",([id], [session['id']]))
                mysql.connection.commit()
                cursor.close()
                return redirect(url_for('dashboard'))
...
def accept_apt(id, aid):
    if 'loggedin' in session:
        if session['username'] != 'admin':
            msg=''
            mess = Message('Message from Admin RentTrack :- Meeting Accepted', sender=app.config["MAIL_USERNAME"])
            cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            if request.method == 'POST':
                cursor.execute('SELECT email FROM property_management.accounts where id = %s;', [id, ])
                data = cursor.fetchall()
                A_ID = aid
                buyer_id = id
                buyer_mail = data[0]["email"]
                Aname = request.form['Aname']
                Fullname = request.form['fullname']
                Email = request.form['email']
                Mobile = request.form['mobile']
                ...
                booking_date = request.form['booking_date']
                # Construct email body for meeting acceptance
                mess.add_recipient(buyer_mail)
                mess.body = f"""\
Hi {Fullname},

We are pleased to inform you that your meeting request has been accepted. Here are the confirmed details:
...
- State: {State}
- Country: {Country}
- Meeting Date: {booking_date}
- Start Time: {starttime}
- End Time: {endtime}

Please make sure to be available at the scheduled time. If you have any questions or need to reschedule, kindly contact us at {app.config["MAIL_USERNAME"]}.
    ...
    if request.method == 'POST':
        ...
        if name and email and message:
            if not re.match(r'[^@]+@[^@]+\.[^@]+', email):
                ...
                cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
                cursor.execute('INSERT INTO contactus (fname, email, message) VALUES (%s, %s, %s)', (name, email, message))
                mysql.connection.commit()
                cursor.close()

                # Sending confirmation email
                admin_email = app.config["MAIL_USERNAME"]
                mes = Message(f'Message from {name}', sender=email, recipients=[admin_email])
                mes.body = f"{message}\n\nFrom:\n{name}\n{email}"
                mail.send(mes)

                user_email = email
                mess = Message('Message from Admin RentTrack', sender=app.config["MAIL_USERNAME"], recipients=[user_email])
                mess.body = f"Hi {name},\n\nThank you for contacting RentTrack. We have received your message and will be in contact soon."
                mail.send(mess)
...
@app.route('/Pay_Rent/<string:id>/<string:cid>', methods=['GET', 'POST'])
def Pay_Rent(id,cid):
...
We are pleased to inform you that your rent payment has been received successfully.  

Payment Details:  
- Applicant ID: {bill_number}  
- Amount: ${amount}  
- Payment Date: {payment_date}  
- Transaction ID: {transaction_id}  

Thank you for your prompt payment. If you have any questions, feel free to contact us.  

Best regards,  
RentTrack Team  
"""
            mess = Message(subject=subject, sender=app.config["MAIL_USERNAME"], recipients=[session['email1']])
            mess.body = body
            mail.send(mess)
            msg = '   Rent has been paid successfully!!'
        else:
            msg = '   Please fill out the form !'
    cur1 = mysql.connection.cursor()
    cur1.execute("SELECT Aname from approvedapart WHERE X_ID=%s", [id,])
    datas = cur1.fetchall()
    mysql.connection.commit()
    cur1.close()
    if 'loggedin' in session:
...
def approve_project():
    ...
    if 'loggedin' in session:
        ...
        if resultValue > 0:
            apply = cur.fetchall()
            cursor = mysql.connection.cursor()
            cursor.execute("SELECT P_ID FROM Buy_project where P_ID in (SELECT P_ID FROM projectdetail WHERE id =%s) GROUP BY P_ID", [session['id']])
            apply2 = cursor.fetchall()
            cursor.close()
            cur.close()  
...
def approve(id, A_ID):
    ...
    Status = 'Approved'
    cur1 = mysql.connection.cursor()
    cur1.execute("INSERT INTO approvedapart VALUES (NULL, %s, (select Aname from apartmentdetail where A_ID=%s),(select fullname from accounts where id=%s),(select email from accounts where id=%s ),(select mobile from accounts where id=%s),0,(select Price from apartmentdetail where A_ID=%s))",[A_ID,A_ID, id,id,id, A_ID])
    mysql.connection.commit()
    cur1.close()
    cursor = mysql.connection.cursor()
    ...
    if resultValue > 0:
        apply = cur.fetchall()
        cursor1 = mysql.connection.cursor()
        cursor1.execute("SELECT A_ID FROM Buy_propertyapt GROUP BY A_ID")
...
def approve2(id, P_ID):
    ...
    if resultValue > 0:
        apply = cur.fetchall()
        cursor1 = mysql.connection.cursor()
        cursor1.execute("SELECT P_ID FROM Buy_project GROUP BY P_ID")
        apply2 = cursor1.fetchall()
        cursor1.close()
        cur.close() 
...
def reject(id,A_ID):
    ...
    mysql.connection.commit()    
    cur1.close()
    cursorml = mysql.connection.cursor()
    cursorml.execute("select email from accounts where id=%s", [id])
    email = cursorml.fetchall()
    cursorml.close()
    cur2 = mysql.connection.cursor()
    cur2.execute("DELETE FROM approvedapart where email=%s and A_ID = %s", [email, A_ID])
    mysql.connection.commit()
    cur2.close()
    cur = mysql.connection.cursor()
    resultValue = cur.execute("SELECT * FROM Buy_propertyapt where A_ID in (SELECT A_ID FROM apartmentdetail WHERE id =%s)",[session['id']])
...
def reject1(id,R_ID):
    ...
    cursorml.close()
    cur2 = mysql.connection.cursor()
    cur2.execute("DELETE FROM approvedroom where email=%s and R_ID = %s", [email, R_ID])
    mysql.connection.commit()
    cur2.close()
    cur = mysql.connection.cursor()
    resultValue = cur.execute("SELECT * FROM Buy_propertyroom where R_ID in (SELECT R_ID FROM roomdetail WHERE id =%s)",[session['id']])
...
def reject2(id,P_ID):
    ...
    cursorml.close()
    cur2 = mysql.connection.cursor()
    cur2.execute("DELETE FROM approvedaproject where email=%s and P_ID = %s", [email, P_ID])
    mysql.connection.commit()
    cur2.close()
    cur = mysql.connection.cursor()
    resultValue = cur.execute("SELECT * FROM Buy_project where P_ID in (SELECT P_ID FROM projectdetail WHERE id =%s)",[session['id']])
...
def Reset_rent():
    try:
        with app.app_context():  # Ensures the function runs within the Flask context
            cur = mysql.connection.cursor()
            cur.execute("SELECT email FROM property_management.accounts")
            mails = cur.fetchall()
            email_list = ", ".join(email[0] for email in mails)
            print(email_list)
...
I hope this email finds you well. This is a friendly reminder that your rent payment for this Month is due on the 7th of this month.

To avoid any late fees, please ensure the payment is made on time. If you have already completed the payment, kindly disregard this message.
...
# FIX: Convert the comma-separated string into a list
            recipient_list = email_list.split(", ")

            mess = Message(subject=subject, sender=app.config["MAIL_USERNAME"], recipients=recipient_list)
            mess.body = body
            mail.send(mess)


            cur1 = mysql.connection.cursor()
            cur1.execute("UPDATE property_management.approvedapart SET Flag = 0;")
            mysql.connection.commit()  # Commit changes to the database
            cur1.close()  # Close cursor after update

            print("Rent reset successfully.")
